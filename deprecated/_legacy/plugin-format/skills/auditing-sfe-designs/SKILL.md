---
name: auditing-sfe-designs
description: Runs a severity-graded compliance audit on a Figma file (or, in code-mode, on React/TypeScript files) against SFE UX standards — accessibility (WCAG 2.2 AA), content design, RAI, design language, and component usage.
triggers:
  - "audit figma"
  - "audit design"
  - "check accessibility"
  - "compliance audit"
  - "review against sfe standards"
when-to-use: "Phase 3 of Cosmo. Also usable standalone via `/agent cosmo` when the user only wants to audit an existing design or codebase."
---
...
- `fluent-agent` MCP is available (preferred) for verifying component-usage findings.
...
Verify component choices via `fluent-agent` MCP. Priority order: `@sfe/react-*` → `@fluentui/react-copilot` → `@fluentui/react-components` → `@fluentui/react-icons`.
...
**MCP status:** figma ✅ · fluent-agent ✅   (or ⚠ with reason)
...
- **Fluent MCP down after one retry:** proceed; mark all component-usage findings with "⚠ verified against local catalog only" and lower confidence.
